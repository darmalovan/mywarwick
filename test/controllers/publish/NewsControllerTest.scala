...
class NewsControllerTest extends BaseSpec with MockitoSugar with Results with MinimalAppPerSuite {
  ...
  when(newsCategoryService.all()).thenReturn(Seq(NewsCategory("abc", "Campus", None, 1)))
  ...
  private val newsController: NewsController = new NewsController(securityServiceImpl, publisherService, newsService, departmentInfoService, audienceBinder, newsCategoryService, userNewsCategoryService, mock[ErrorHandler], audienceService, userPreferencesService) {
  ...
  "NewsController#audienceInfo" should {
    ...
    "respond with audience size" in {
      ...
      when(newsCategoryService.getNewsCategoryForCatId("abc")).thenReturn(NewsCategory("abc", "Campus", None, 1))
